DELETE FROM mark WHERE student_id IN (SELECT DISTINCT s.id FROM STUDENT s JOIN mark m ON s.id = m.student_id JOIN subje ss ON m.subject_id = ss.id WHERE ss.grade > 4);
